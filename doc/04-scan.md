...
- [*知识点1: SliceTransform 与前缀扫描三模式*](#id1)
...
**前缀扫描的终止语义 = Seek 时把目标前缀存进 `prefix_`，之后每步 Next/Prev 先做一次 PrefixCheck：`Transform(当前 key)` 不等于记下的前缀，立刻 `valid_ = false`——扫到第一个不同前缀的 key 就停。**
...
| 前缀扫描怎么停下来？ | Seek 时把目标前缀存进 `prefix_`，之后每步 PrefixCheck：`Transform(key)` 不等于记下前缀就 `valid_ = false`；入口另有 prefix bloom 拦"前缀不存在"的 seek（03 KP2）——一个管开始，一个管停 |
...
**记忆口诀**：**"Transform 切前缀，InDomain 判域；Seek 记 prefix，越界即止步；upper 围墙 lower 地板，界内自证省比较。"**

---

**下一站**：扫描的边界语义讲完了。回到数据本身——MemTable 这个被迭代、被扫描的容器，什么时候写满冻结、active/immutable 怎么切换、内存账记到多少触发 flush、并发写怎么组织。→ 05-memtable（待写）
