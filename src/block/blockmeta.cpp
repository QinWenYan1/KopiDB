...
// Lab 3.4将内存中所有`Blcok`的元数据编码为二进制字节数组
void BlockMeta::encode_meta_to_slice(std::vector<BlockMeta> &meta_entries,
  ...
  // MetaEntry = offset(32) | first_key_len(16) | first_key | last_key_len(16) |
  // last_key
  ...
  for (const auto &m : meta_entries)
    total += sizeof(uint32_t) + sizeof(uint16_t) * 2 + m.first_key.size() +
             m.last_key.size();
  ...
  uint32_t hash =
      static_cast<uint32_t>(std::hash<std::string_view>{}(entries_view));
  memcpy(ptr, &hash, sizeof(uint32_t));
}

// Lab 3.4将二进制字节数组解码为内存中的`Blcok`元数据
...
BlockMeta::decode_meta_from_slice(const std::vector<uint8_t> &metadata) {
  ...
  if (metadata.size() < sizeof(uint32_t) * 2)
    throw std::runtime_error(
        "BlockMeta::decode_meta_from_slice: Invalid metadata size");
  ...
  if (expect != actual)
    throw std::runtime_error(
        "BlockMeta::decode_meta_from_slice: Metadata hash mismatch");
